...
class HeadPoseAnalyzer:
    ...
    def __init__(
        self, head_up_threshold: float = -10.0, head_down_threshold: float = 15.0
    ):
        """
        初始化头部姿态分析器
    ...
    def _get_keypoint(
        self, keypoints: List[Dict], index: int
    ...
    def calculate_head_angle(self, keypoints: List[Dict]) -> Optional[float]:
        ...
        left_shoulder = self._get_keypoint(
            keypoints, self.KEYPOINT_INDICES["left_shoulder"]
        )
        right_shoulder = self._get_keypoint(
    ...
    def get_statistics(self, detections_with_pose: List[Dict]) -> Dict:
        ...
        return {
            ...
            "head_down_percentage": (
                round(head_down / total * 100, 1) if total > 0 else 0
            ),
        }
